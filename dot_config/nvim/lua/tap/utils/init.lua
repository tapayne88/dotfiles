...
---@return Job
utils.get_os_command_output_async = a.wrap(
                                        function(cmd, cwd, fn)
...
function utils.require_plugin(name, callback)
    if not string.match(name, '^tap%.plugins%..+') then
        vim.notify(string.format("Attempted to load non-plugin! [%s]", name),
                   "error", {title = "require_plugin"})
        return nil
    end
...
function utils.apply_user_highlights(name, callback, _opts)
    ...
    if not force and has_augroup(augroup_name) then
        vim.notify("augroup already exists with name " .. augroup_name, "error",
