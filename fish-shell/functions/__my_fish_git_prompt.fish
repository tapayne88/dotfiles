...
function __my_fish_git_prompt --description "Prompt function for Git"
...
#	test -n "$git_dir"; or return
        ...
        if test -z "$git_dir"
            echo ' '(__my_fish_separator normal blue)' '
            return
        end
...
#		set s "$___fish_git_prompt_color_stashstate$s$___fish_git_prompt_color_stashstate_done"
	...
	if test -n "$u"
#		set w "$___fish_git_prompt_color_untrackedfiles$u$___fish_git_prompt_color_untrackedfiles_done"
		set w "$u"
	end
	set -l f "$w$i$s"
	set -l format $argv[1]
	...
	if test -n "$f"
            set -g __my_fish_prompt_colour 'yellow'
	    set f "$f"
        else
            set -g __my_fish_prompt_colour 'green'
	end
...
#	printf "%s$format%s" "$___fish_git_prompt_color_prefix" "$___fish_git_prompt_color_prefix_done$c$b$f$r$p$___fish_git_prompt_color_suffix" "$___git_ps_color_suffix_done"
	printf "%s$format%s" (__my_fish_separator $__my_fish_prompt_colour blue) (set_color --background=$__my_fish_prompt_colour black) "$___fish_git_prompt_char_prefix $c$b$f$r$p " (__my_fish_separator normal $__my_fish_prompt_colour)
end
...
function __fish_git_prompt_validate_chars --description "__fish_git_prompt helper, checks char variables"
	...
	if not set -q ___fish_git_prompt_char_upstream_equal
		set -g ___fish_git_prompt_char_upstream_equal (set -q __fish_git_prompt_char_upstream_equal; and echo $__fish_git_prompt_char_upstream_equal)
	end
